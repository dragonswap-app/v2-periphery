...
import '@dragonswap/v2-core/contracts/interfaces/callback/IDragonswapV2FlashCallback.sol';
import '@dragonswap/v2-core/contracts/libraries/LowGasSafeMath.sol';
...
/// @notice An example contract using the Dragonswap V2 flash function
contract PairFlash is IDragonswapV2FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `dragonswapV2FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IDragonswapV2Pool pool = IDragonswapV2Pool(PoolAddress.computeAddress(factory, poolKey));
